...
export function ConsentGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {/* Always render children — modal overlays on top when needed */}
      {children}

      {/* Show modal overlay only when mounted and not yet accepted */}
      {mounted && !accepted && (
        <>
          <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-sm z-[9998]" />
